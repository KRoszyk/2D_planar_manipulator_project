...
"""
Here you can switch your controller
"""
...
def system(x, t):
    ...
    control = controller.calculate_control(x, q_d_ddot[:, np.newaxis], q_d[:, np.newaxis], q_d_dot[:, np.newaxis], q_d_ddot[:, np.newaxis])
    ctrl.append(control)
...
x = odeint(system, np.concatenate([q_d, q_d_dot], 0), t, hmax = 1e-2)
manipulator.plot(x)
